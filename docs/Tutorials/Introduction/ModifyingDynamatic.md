...
If you have deleted the data generated by the synthesis flow on this kernel, you can regenerate it fully using the [`loop-accumulate.dyn` frontend script](../../../tutorials/Introduction/Ch2/loop-accumulate.dyn) that has already been written for you. Just run the following command from Dynamatic's top-level folder.
...
Done! Now you can re-run the same frontend script as earlier (`./bin/dynamatic --run tutorials/Introduction/Ch2/loop-accumulate.dyn`) to see the results of your work! Note that the circuit still functionally verifies during the `simulate` step as the frontend prints `[INFO] Simulation succeeded`.
